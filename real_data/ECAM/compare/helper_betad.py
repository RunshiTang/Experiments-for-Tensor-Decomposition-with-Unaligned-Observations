...
def trivial_loss(microbiome, beta = -0.5, bias_parameter = 10**(-10)):
    ...
    for b_index in range(p):
        for a_index in range(n):
            for xi_index in range(dim_Ti_list[a_index]):
                sum_l += Beta_divergnece_loss_f(0, microbiome[a_index][b_index+1][xi_index])
                
    return sum_l/(p*sum_Ti)
...
def RKHS_generalized_sketch_microbiome_beta_divergnece(microbiome, R = 5, beta = 1.5,
    ...
    for epoch in range(n_max):
        ...
        if stop & (len(loss) > (stop_steps+1)):
            fit_last0 = np.flip(loss_array)[0:(stop_steps)]
            fit_last1 = np.flip(loss_array)[1:(stop_steps+1)]
            if np.all(fit_last0 > fit_last1 - 10**(-4)):
                stopped = True
                final_loss = evaluation
                return [[A_list[-stop_steps-1], B_list[-stop_steps-1], theta_list[-stop_steps-1]], final_loss, loss, stopped, time_list]
