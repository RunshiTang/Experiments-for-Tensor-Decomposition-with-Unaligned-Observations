...
import os
os.chdir("//sscwin/dfsroot/Users/rtang56/Desktop/RKHS_new/RKHS/simulation/setting2")
...
#acc_list_list = 0.9837724 - acc_list_list
...
#acc_unsketched = 0.9837724-acc_unsketched
...
plt.axhline(y = 0.9837724, linestyle = ':', color = "#17becf", linewidth = 0.5)
...
for j in range(len(s1_list)):
    ...
    ax.plot(time_mean, acc_mean, 
                     label ="S-RKHS, S1=" + str(s1_list[j]), 
                     linestyle='solid', color=color[j+1], linewidth=3)
...
plt.savefig('comparison_partial.pdf', format = 'pdf')
plt.clf()
...
plt.ylabel('Fit')
plt.tight_layout()
plt.savefig('loss_iteration_partial.pdf', format = 'pdf')
plt.clf()
...
data_concatenated = []
data = np.concatenate((time_unsketched[np.newaxis,:], time_list_list))[:,:,1:16]
