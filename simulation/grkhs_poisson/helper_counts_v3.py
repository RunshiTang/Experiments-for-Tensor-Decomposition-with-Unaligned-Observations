...
def RKHS_generalized_sketch_microbiome_counts(microbiome, R = 5, 
    ...
    for epoch in range(n_max):
        ...
        for iterator in range(iterations_per_epoch):
            ...
            for r in range(R):
                ...
                if norm_prod > lambda_parameter:
                    theta_tmp = theta_tmp/theta_norm_tmp * (lambda_parameter**(1/3))
                    theta_new[r*len(T):(r+1)*len(T),:] = theta_tmp
                    A_new[:,r] = A_new[:,r] / a_norm_tmp * (lambda_parameter**(1/3))
                    B_new[:,r] = B_new[:,r] / b_norm_tmp * (lambda_parameter**(1/3))
        ...
        if stop & (len(loss) > (stop_steps+1)):
            loss_last0 = np.flip(loss_array)[0:(stop_steps)]
            loss_last1 = np.flip(loss_array)[1:(stop_steps+1)]
            if np.all(loss_last0 > loss_last1 - 10**(0)):
                stopped = True
                break
...
def RKHS_generalized_microbiome_counts(microbiome, R = 5, 
                    lambda_parameter = 1000,
                    leanring_rate = 0.0001, 
                    n_max = 10, 
                    bad_epochs_max = 3,
